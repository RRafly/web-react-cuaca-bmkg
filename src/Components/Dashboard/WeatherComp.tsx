...
function WeatherCard({ weatherData }: { weatherData: Cuaca }) {
    ...
    return (
        <div className={`shadow-md flex flex-col grow rounded-md items-center p-3 bg-gray-700 text-white`}>
            ...
            <h3 className="text-4xl font-semibold -mt-3 ml-3">{weatherData.t}<sup className={`${styles["sup-celcius"]} text-base font-bold text-gray-300`}>&deg;C</sup></h3>
            <p className="text-xl font-normal text-gray-300 -mt-2">{weatherData.weather_desc ?? "Cerah Sekali"}</p>

            <div className="flex flex-row gap-4 mt-1 pt-4 w-full justify-between border-t-2 border-gray-500">
...
function WeatherNowCard({ weatherData }: { weatherData: Cuaca }) {
    return (
        <div className={`shadow-md flex flex-col grow rounded-md items-center p-4 bg-gray-700 text-white`}>
            <h1 className="text-4xl font-medium mb-2 text-gray-200">Sekarang</h1>
            <div className="flex flex-row">
                <img className="w-24 h-[96px]" src={weatherData.image ?? "https://api-apps.bmkg.go.id/storage/icon/cuaca/cerah berawan-pm.svg"} />
                <div className="flex flex-col justify-center">
                <h3 className="text-7xl tracking-tighter font-semibold">{weatherData.t}<sup className={`${styles["sup-celcius-6xl"]} font-bold text-gray-300 text-2xl tracking-normal`}>&deg;C</sup></h3>
                </div>
            </div>

            <p className="font-medium text-3xl -mt-3">{weatherData.weather_desc ?? "Cerah Sekali"}</p>

            <div className="flex flex-row gap-4 text-xl font-medium border-t-2 border-gray-500 pt-4 mt-2">
                <div className="flex flex-col gap-1">
                    <small>Kelembapan</small>
                    <small>Angin</small>
                </div>
...
function DayCard({ cuaca }: { cuaca: Cuaca[] | undefined }) {
    ...
    return (
        <div className="shadow-md p-4 me-3 rounded-md bg-gray-800 text-white flex flex-col gap-3">
            <div>
